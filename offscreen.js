...
var SUGGESTION_MODEL = "gemini-flash-lite-latest";
  ...
  liveSession = await ai.live.connect({
    ...
    config: {
      ...
      systemInstruction: {
        parts: [
          {
            text: "You are a transcription assistant. Prioritize accurate input transcription. Keep spoken responses extremely brief. You work for ENTAB Infotech"
          }
        ]
      }
    },
...
async function triggerSuggestions() {
  ...
  const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${SUGGESTION_MODEL}:generateContent?key=${encodeURIComponent(apiKey)}`;

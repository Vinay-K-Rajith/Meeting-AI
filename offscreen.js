...
var outputSuggestionTranscript = "";
var finalizedSuggestions = [];
var pendingPcmSamples = [];
var PCM_BATCH_SAMPLES = 4e3;
var LIVE_MODEL = "gemini-3.1-flash-live-preview";
var LIVE_MODEL_FALLBACK = "gemini-2.5-flash-native-audio-preview-12-2025";
var currentLiveModel = LIVE_MODEL;
var liveSession = null;
var isLiveReady = false;
var reconnectTimerId = null;
var reconnectAttempt = 0;
var MAX_RECONNECT_ATTEMPTS = 8;
var isConnectingLive = false;
var liveConnectionToken = 0;
var ENTAB_KNOWLEDGE_BASE = "Entab Infotech is an Indian EdTech company with 23+ years in school ERP. Its flagship platform is CampusCare, a cloud-based school management ERP used by 1200+ schools. Core modules include student registration and lifecycle records, fee management and online collection, exam and assessment workflows, lesson planning and assignments, parent-teacher communication portals, library, HR, payroll, inventory, attendance, and bus GPS tracking. The platform supports NEP 2020 aligned structures and competency-focused assessment, and provides analytics dashboards for data-driven school decisions.";
  ...
  liveSession = await ai.live.connect({
    ...
    config: {
      ...
      systemInstruction: {
        parts: [
          {
            text: "You are a live meeting copilot for ENTAB Infotech. After each user turn, generate exactly 3 high-quality response suggestions the user can say next. The suggestions must be direct answers/statements the user can speak immediately, not follow-up questions. Use assertive, professional, helpful language in business conversations. Each suggestion must be one line, natural spoken English, under 18 words, and non-repetitive. Never use question marks. Never ask the other person another question. Prefer grounded answers based on this product knowledge: " + ENTAB_KNOWLEDGE_BASE + " Output format must be exactly:\n1) <suggestion>\n2) <suggestion>\n3) <suggestion>\nDo not output anything except these 3 lines."
          }
        ]
      }
    },
...
function scheduleReconnect(detail) {
  ...
  const delayMs = Math.min(1e3 * reconnectAttempt, 5e3);
  forwardMessage({
    type: "CAPTURE_STATE",
    tabId: activeTabId,
    state: "RECONNECTING"
  ...
  forwardMessage({
    type: "ASSISTANT_ERROR",
    tabId: activeTabId,
    error: `${detail} Retrying in ${Math.round(delayMs / 1e3)}s with ${currentLiveModel}.`
  });
  clearTimeout(reconnectTimerId);
...
function extractSuggestionLines(text) {
  ...
  const lines = text.replace(/\r/g, "\n").split("\n").map((line) => normalizeSuggestionLine(line)).filter(Boolean).map((line) => line.replace(/^\d+\)\s*/, "").trim());
  if (lines.length >= 2) {
    return uniqueLastN(lines, 3);
  }
  const byNumbering = text.split(/\s(?=\d+\))/g).map((part) => normalizeSuggestionLine(part).replace(/^\d+\)\s*/, "").trim()).filter(Boolean);
  if (byNumbering.length >= 2) {
    return uniqueLastN(byNumbering, 3);
  }
  const bySentence = text.split(/[.!?]\s+/).map((part) => normalizeSuggestionLine(part)).filter(Boolean);
